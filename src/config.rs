...
/// CLI subcommands for non-server operations.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    /// Run the vico-vee HTTP service (default).
    Server,
    /// Create a backup of the data directory.
    Backup {
        /// Path to write the backup tarball to.
        #[arg(long)]
        output: Option<PathBuf>,
    },
    /// Restore the data directory from a backup archive.
    Restore {
        /// Path to the backup tarball to restore from.
        #[arg(long)]
        input: PathBuf,
    },
}
